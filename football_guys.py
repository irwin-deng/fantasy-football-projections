...
def _get_position_df(week, position):
    ...
    df = _read_position_page(week, position)
    ...
    if position == "qb":
        ...
        expected_labels = ["NAME", "TEAM", "OPP", "POS", "RSH", "YD", "TD", "REC", "YD", "TD", "FPT"]
        ...
        new_labels = ["Name", "Team", "Opp", "Pos", "FG Made", "FG Att", "XP Made", "XP Att", "FPT"]
    elif position == "td":
        ...
        new_labels = ["Name", "Team", "Opp", "Pos", "IDP Tkl", "IDP Ast", "IDP Sack",
                      "IDP FF", "IDP FR", "IDP Int", "IDP PD", "IDP TD", "FPT"]
    else:
...
def _read_position_page(week, position):
    """
    scrapes projections from FootballGuys
    ...
    if len(tables) != 1:
        raise Exception("{num} tables found on url: {url}").format(num=len(tables), url=url)
    table = tables[0]
    
    df = util_scripts.get_dataframe_from_table_tr_headers(table)
    
    # the first col is indices. Drop that
